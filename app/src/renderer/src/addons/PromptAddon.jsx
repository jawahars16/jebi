...
export class PromptAddon {
  ...
  getStickyCommand(viewportY) {
    ...
    if (!best) return null
    return { command: best.command, cwd: best.cwd, exitCode: best.exitCode, onCopy: best.onCopy }
  }
  ...
  updateLastCwd(cwd) {
    ...
    entry.cwd = cwd
    entry.root?.render(
      <Prompt command={entry.command} cwd={cwd} exitCode={entry.exitCode} rowHeight={entry.cellHeight} onCopy={entry.onCopy} />
    )
  }
  ...
  updateLastExitCode(code) {
    ...
    entry.exitCode = code
    entry.root?.render(
      <Prompt command={entry.command} cwd={entry.cwd} exitCode={code} rowHeight={entry.cellHeight} onCopy={entry.onCopy} />
    )
  }
  ...
  commandStart(command, cwd = '') {
    ...
    const cellHeight =
      ...
      (this._term.element ? this._term.element.offsetHeight / this._term.rows : 25)
    const promptRows = PROMPT_HEADER_ROWS + commandLines
    ...
    const entry = { marker, command, cwd, exitCode: 0, root: null, cellHeight, onCopy: null }
    ...
    decoration.onRender(el => {
      ...
      el.style.backgroundColor =
        getComputedStyle(document.documentElement).getPropertyValue('--bg-surface').trim() || '#141416'
      ...
      if (!el._reactRoot) {
        ...
        root.render(
          <Prompt command={command} cwd={cwd} exitCode={entry.exitCode} rowHeight={cellHeight} onCopy={entry.onCopy} />
        )
      }
    })
